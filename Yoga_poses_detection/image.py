...
def draw_labels(ax,
    ...
    for i, (pos, label, yoga_label) in enumerate(zip(positions, labels, yoga)):
        if class_names is not None:
            ...
            if label == 0 and yoga_label is None:
                label_text = 'not yoga '+ class_names[label]
            if label != 0:
                label_text = class_names[label] #f'class {label}'
...
effnet_b0_yoga = pickle.load(open('yoga_poses_classificator_model.pkl', 'rb'))
